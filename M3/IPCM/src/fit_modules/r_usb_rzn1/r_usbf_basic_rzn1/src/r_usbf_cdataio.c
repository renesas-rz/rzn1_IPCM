...
uint16_t usb_cstd_write_data(uint16_t pipe, uint16_t pipemode)
{
    ...
    {
        /* Max Packet Size */
        mxps = usbf_eps[ep_num].max_packet_size;
        ...
        if (count > 0)
        {
            ...
            /*
             * Send one more packet to non-control endpoint as either needs a short packet or a NULL
             * packet to be sent to terminate the transfer.
             * Or it is the control endpoint which does not need NULL packet but *may* require a short packet.
             */
            if ( (USB_EP0 != ep_num) || (count % mxps) )
            {
                pkts_to_send++;
            }
            ...
            while (pkts_to_send)
            {
                ...
                } else
                {
                    if (0 == pkt_size)
                    {
                        /* NULL packet so wait for EPm_IN_EMPTY bit to clear */
                        for (i = 800; i > 0; i--)
                        {
                            if (epm_status_reg_addr->EPm_STATUS.BIT.EPm_IN_EMPTY)
                            {
...
/***********************************************************************************************************************
 *
 *  Function Name:  usb_cstd_SelectNak
 *
