...
static ER_RET HW_Init(void)
{
    ...
    } else
    {
        ...
        if (ER_OK == ret_val)
        {
                    ...
                    (ep_num <= USB_MAX_EP_NO) && (ER_OK == ret_val); ep_num++)
            {
                        ...
                        & (0x01 << ep_num))
                {
                    ...
                    if ( USB_ERROR == pipe)
                    {
                        /* pipe not configured for this EP and direction */
        ...
        /* Delay_usec(TIME_DETACH_DETECT_US);*/
...
uint16_t usb_pstd_ChkVbsts(void)
{
    ...
    do
    {
        ...
        Delay_usec(10); /* delay for 10 usecs */
        buf2 =
                ((volatile AHB_EPC_EPCTR_Type *) USBFUNC_EPCTR_ADDRESS)->EPCTR.BIT.VBUS_LEVEL;
        Delay_usec(10); /* delay for 10 usecs */
        buf3 =
...
void handle_usbf_epc_isr(IRQn_Type *irq_num_ptr)
{
    ...
    uint16_t ep_num;
    volatile EPC_USB_INT_STA_Type *usb_int_status_reg_addr = (volatile EPC_USB_INT_STA_Type *) (USBFUNC_USB_EPC_USB_INT_STA_ADDRESS);
    volatile EPC_EP0_STATUS_Type *ep0_status_reg_addr;
    volatile EPC_EPm_STATUS_Type *epm_status_reg_addr;
    ...
    if (RZN1_IRQ_USBF_EPC == irq_num)
    {
        ...
        if (0 == usb_int_status_reg_addr->USB_INT_STA.LONG)
        {
            return;
        }
#endif
